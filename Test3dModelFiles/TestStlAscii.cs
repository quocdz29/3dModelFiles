...
namespace Test3dModelFiles
{
    ...
    public sealed class TestStlAscii
    {
        ...
        [TestMethod]
        [DataRow(data: "TestFiles\\Tetrahedron-ASCII.stl")]
        public void TestLoad(string filename)
        {
            ...
            CommonFileData results = STLFile.Ascii.Load(file);
        ...
        {
            FileInfo originalFileInfo = new FileInfo("TestFiles\\Tetrahedron-ASCII.stl");
            FileInfo reproducedFileInfo = new FileInfo( Path.GetFullPath("Test_ASCII.stl"));

            CommonFileData original_FileData = STLFile.Ascii.Load(originalFileInfo);
       
            bool result = STLFile.Ascii.Save(original_FileData, reproducedFileInfo);
            ...
            Assert.IsTrue(result, "STLFile.Ascii.Save result.");
            ...
            CommonFileData reproduced_FileData = STLFile.Ascii.Load(reproducedFileInfo);
            ...
            TestContext.WriteLine("");
            TestContext.WriteLine("--------------------------------------");
            TestContext.WriteLine("File.Data:");
            TestContext.WriteLine(fileData);
        }
